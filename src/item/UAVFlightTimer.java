...
public class UAVFlightTimer {
    ...
    public void start(Uav uav) {
        if (!isTiming) {
            ...
            this.isTiming = true;
            System.out.println("No." + uav.getId() + "の飛行タイマーが開始されました。");
        }
    }
    ...
    public void stop(Uav uav) {
        if (isTiming) {
            ...
            isTiming = false;
            System.out.println("No." + uav.getId() + "の飛行タイマーが停止されました。累積飛行時間: " + totalElapsedTime / 1000 + " s");
        }
    }
    ...
    public void cancel(Uav uav) {
        ...
        System.out.println("No." + uav.getId() + "が目的地に到着しました。");
    }
}
