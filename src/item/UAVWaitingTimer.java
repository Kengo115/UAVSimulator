...
public class UAVWaitingTimer {
    ...
    public void start(Uav uav) {
        if (!isTiming) {
            ...
            this.isTiming = true;
            System.out.println("No." + uav.getId() + "の待機タイマーが開始されました。");
        }
    }
    ...
    public void stop(Uav uav) {
        if (isTiming) {
            ...
            isTiming = false;
            System.out.println("No." + uav.getId() + "の待機タイマーが停止されました。累積待機時間: " + totalElapsedTime / 1000 + " s");
        }
    }
    ...
    public void cancel(Uav uav) {
        ...
        System.out.println("No." + uav.getId() + "の待機タイマーがキャンセルされました。");
    }
}
