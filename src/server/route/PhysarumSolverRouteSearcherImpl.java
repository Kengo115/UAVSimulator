...
/**
 * 粘菌アルゴリズム（Physarum Solver）による経路探索を行う実装クラス
 */
public class PhysarumSolverRouteSearcherImpl implements PhysarumSolverRouteSearcher {
     ...
     */
    public PhysarumSolverRouteSearcherImpl(Link[][] linkMatrix, int nodeNum, ResultOutputManager outputManager, 
    ...
    public void searchAndAssignRoutes(Client client, Queue<Uav> flyingUavQueue, Queue<Uav> uavQueue, int numLoop) throws IOException {
        run_PS(client, flyingUavQueue, uavQueue, null, numLoop);
    }
    ...
    public void run_PS(Client client, Queue<Uav> flyingUavQueue, Queue<Uav> uavQueue, ClientController clientController, int numLoop) throws IOException {
        ...
        while (ct < numLoop) {
            ...
            // チューブ厚の更新
            updateTubeThickness(nodeNum, degeneracyEffect, config.getDeltaTime());
    ...
    public void updateTubeThickness(int nodeNum, double degeneracyEffect, double deltaTime) {
        for (int i = 0; i < nodeNum; i++) {
            for (int j = 0; j < nodeNum; j++) {
                if (linkMatrix[i][j].getL_tubeLength() != config.getInf()) {
                    double deltaThickness = (qTubeFlowSigmoidOutput[i][j] - (degeneracyEffect * linkMatrix[i][j].getD_tubeThickness())) * deltaTime;
