...
def load_inflight_wait_times(time_dir: str) -> list:
    """全client分のflight_times.csvを読み込み、飛行中待機時間のリストを返す"""
    ...
    for csv_path in csv_files:
        ...
        # 飛行中待機時間 = waitingTime
        inflight_wait_times.extend(df['waitingTime'].tolist())

    return inflight_wait_times


def interactive_input():
    """対話的に入力を受け取る"""
    print("=" * 80)
    print("飛行中待機時間 CDF比較グラフ生成")
    print("=" * 80)
    print()
    ...
    # 出力ファイル名
    default_output = "output/inflight_wait_cdf_comparison.png"
...
def load_data(groups, base_dir="src/result"):
    ...
    for group in groups:
        ...
        # データ読み込み
        inflight_wait_times = load_inflight_wait_times(time_dir)

        if not inflight_wait_times:
            print(f"警告: {group['display_name']} (sim_{sim_num}) のデータが読み込めませんでした")
            print(f"      パス: {time_dir}")
        else:
            print(f"✓ {group['display_name']} (sim_{sim_num}) データ読み込み成功: {len(inflight_wait_times)} UAVs")

        all_data.append({
            'display_name': group['display_name'],
            'values': sorted(inflight_wait_times) if inflight_wait_times else []
        })
...
def plot_cdf(all_data, x_max, x_interval, output_file):
    ...
    # X軸設定
    ax.set_xlabel('In-flight Waiting Time [s]', fontsize=FONT_SIZE_TITLE, fontweight='bold', color='black')
