...
def load_distances(time_dir: str) -> list:
    """全client分のflight_times.csvを読み込み、distanceのリストを返す"""
...
def interactive_input():
    ...
    # 出力ファイル名
    default_output = "output/distance_cdf_comparison.png"
...
def load_data(groups, base_dir="src/result"):
    ...
    for group in groups:
        ...
        # データ読み込み
        distances = load_distances(time_dir)

        if not distances:
            print(f"警告: {group['display_name']} (sim_{sim_num}) のデータが読み込めませんでした")
            print(f"      パス: {time_dir}")
        else:
            print(f"✓ {group['display_name']} (sim_{sim_num}) データ読み込み成功: {len(distances)} UAVs")

        all_data.append({
            'display_name': group['display_name'],
            'values': sorted(distances) if distances else []
        })
...
def plot_cdf(all_data, x_max, x_interval, output_file):
    ...
    # X軸設定
    ax.set_xlabel('Flight distance [m]', fontsize=FONT_SIZE_TITLE, fontweight='bold', color='black')
