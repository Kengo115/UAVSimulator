...
def load_preflight_wait_times(time_dir: str) -> list:
    """全client分のflight_times.csvを読み込み、飛行前待機時間のリストを返す"""
    ...
    preflight_wait_times = []

    for csv_path in csv_files:
        df = pd.read_csv(csv_path)
        # 飛行前待機時間 = pathWaitTime + flightStayingTime
        wait_times = df['pathWaitTime'] + df['flightStayingTime']
        preflight_wait_times.extend(wait_times.tolist())

    return preflight_wait_times


def interactive_input():
    """対話的に入力を受け取る"""
    print("=" * 80)
    print("飛行前待機時間 CDF比較グラフ生成")
    print("=" * 80)
    print()
    ...
    # 出力ファイル名
    default_output = "output/preflight_wait_cdf_comparison.png"
...
def load_data(groups, base_dir="src/result"):
    ...
    for group in groups:
        ...
        # データ読み込み
        preflight_wait_times = load_preflight_wait_times(time_dir)

        if not preflight_wait_times:
            print(f"警告: {group['display_name']} (sim_{sim_num}) のデータが読み込めませんでした")
            print(f"      パス: {time_dir}")
        else:
            print(f"✓ {group['display_name']} (sim_{sim_num}) データ読み込み成功: {len(preflight_wait_times)} UAVs")

        all_data.append({
            'display_name': group['display_name'],
            'values': sorted(preflight_wait_times) if preflight_wait_times else []
        })
...
def plot_cdf(all_data, x_max, x_interval, output_file):
    ...
    # X軸設定
    ax.set_xlabel('Pre-flight Wait Time [s]', fontsize=FONT_SIZE_TITLE, fontweight='bold', color='black')
