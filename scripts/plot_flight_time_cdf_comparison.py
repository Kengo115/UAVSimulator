#!/usr/bin/env python3
"""
flightTime CDF比較グラフ生成スクリプト

複数の経路探索手法を同一グラフ上で比較する累積分布関数(CDF)グラフを生成します。
横軸: 時間(秒)、縦軸: 累積割合(%)

flightTime = CSV上の flightTime + pathWaitTime
"""
...
def load_flight_times(time_dir: str) -> list:
    """全client分のflight_times.csvを読み込み、flightTimeのリストを返す"""
...
def interactive_input():
    ...
    # 出力ファイル名
    default_output = "output/flight_time_cdf_comparison.png"
...
def load_data(groups, base_dir="src/result"):
    ...
    for group in groups:
        ...
        # データ読み込み
        flight_times = load_flight_times(time_dir)

        if not flight_times:
            print(f"警告: {group['display_name']} (sim_{sim_num}) のデータが読み込めませんでした")
            print(f"      パス: {time_dir}")
        else:
            print(f"✓ {group['display_name']} (sim_{sim_num}) データ読み込み成功: {len(flight_times)} UAVs")

        all_data.append({
            'display_name': group['display_name'],
            'values': sorted(flight_times) if flight_times else []
        })
